...
;;;
;;; $Id: net.lisp,v 1.8 2005/06/21 16:02:59 dvd Exp $
;;; $Source: /home/dvd/srv/CVSROOT/CL-UTILS/port/net.lisp,v $
...
(defun open-socket-server (&optional port)
  ...
  #+lispworks (let ((mbox (mp:make-mailbox :size 1)))
                (make-socket-server
                 :mbox mbox :port port
                 :proc (#+win32 comm:start-up-server 
								 				#-win32 comm:start-up-server-and-mp
...
Returns a socket stream or NIL."
  ...
  #+openmcl (ccl:accept-connection serv :wait (not (eql wait 0))) ; in OpenMCL nil is no wait
  #+(and sbcl db-sockets)
